...
contract CampaignManager {
    ...
    function createCampaign(string memory _campaignCID, uint256 _target) public returns(bool) {
    uint256 campaignID = campaignIdCounter;
    campaignIdCounter++;
    Campaign campaign = new Campaign(msg.sender, _campaignCID, block.timestamp, _target, campaignID);
    ...
    function getAllCampaignAddresses () public view returns(address[] memory _campaigns) {
    ...
    function getCampaignDetails(address[] calldata _campaignList) public view returns(string[] memory campaignCID, address[]memory owner, uint256[] memory id, uint256[] memory raisedFunds){
